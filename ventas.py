...
class SelectableBoxLayout(RecycleDataViewBehavior, BoxLayout):
    ...
    def refresh_view_attrs(self, rv, index, data):
    	...
    	self.ids['_precio_por_unidad'].text = str("{:.2f}".format(data['precio']))
...
class RV(RecycleView):
    ...
    def modificar_articulo(self):
    	indice = self.articulo_seleccionado()
    	if indice >= 0:
    		popup = EditarCantidadPopup(self.data[indice], self.actualizar_articulo)
    		popup.open()
    ...
    def actualizar_articulo(self, valor):
    	...
    	if indice >= 0:
    		...
    		self.editar_cantidad(False, nuevo_total)
...
class EditarCantidadPopup(Popup):
	def __init__(self, data, actualizar_articulo_callback, **kwargs):
		super(EditarCantidadPopup, self).__init__(**kwargs)
		self.data = data
	...
	def validar_input(self, text_input):
		try:
			nueva_cantidad = int(text_input)
...
class PopupPago(Popup):
	def __init__(self, total, pagado_callback, **kwargs):
		super(PopupPago, self).__init__(**kwargs)
		self.total = total
...
class NuevaCompraPopup(Popup):
	def __init__(self, n_compra_callback, **kwargs):
		super(NuevaCompraPopup, self).__init__(**kwargs)
		self.n_compra = n_compra_callback
...
class VentasWindow(BoxLayout):
	...
	def busqueda_codigo(self, codigo):
		...
		for producto in inventario_sql:
			if codigo == producto[0]:
				...
				self.agregar_producto(articulo)
				self.ids.BCodigo.text=''
				break

	def busqueda_nombre(self, nombre):
		self.ids.BNombre.text = ''
	...
	def editar_cantidad(self, cambio = True, nuevo_total = None):
		if cambio:
	...
	def pagar(self):
		if self.ids.rvs.data:
			popup = PopupPago(self.total, self.pagado)
			popup.open()
		else:
			self.ids.noti_fallo.text = 'No hay nada en el carrito'

	def pagado(self):
		self.ids.noti_exito.text = 'Compra concluida'
		self.ids.noti_fallo.text = ''
		self.ids.total.text = "{:.2f}".format(self.total)
		self.ids.BCodigo.disabled = True
		self.ids.BNombre.disabled = True
	...
	def n_compra(self, desde_popup = False):
		if desde_popup:
			self.ids.rvs.data = []
			self.total = 0.0
			self.ids.sub_total.text = '0.00'
			self.ids.total.text = '0.00'
			self.ids.noti_exito.text = ''
			self.ids.noti_fallo.text = ''
			self.ids.BCodigo.disabled = False
			self.ids.BNombre.disabled = False
			self.ids.pagar.disabled = False
			self.ids.rvs.refresh_from_data()
		elif len(self.ids.rvs.data):
			popup = NuevaCompraPopup(self.n_compra)
			popup.open()
	...
	def cerrarsesion(self):
		if self.ids.rvs.data:
			self.ids.noti_fallo.text = 'Compra inconclusa'
		else:
